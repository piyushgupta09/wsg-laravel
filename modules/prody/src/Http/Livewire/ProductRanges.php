...
class ProductRanges extends Component
{
    ...
    {
        $this->ranges = config('panel.sizes');
        $this->showForm = false;
    ...
    public function resetForm()
    {
        ...
        foreach ($this->productMaterials as $productMaterial) {
            ...
            $this->consumption[$productMaterial->id . '_0']['cost'] = null;
            $this->consumption[$productMaterial->id . '_0']['unit'] = config('panel.units.fcpu')['inch'];
        ...
        try {
            ...
            foreach ($this->consumption as $key => $value) {
                ...
                if (Str::after($key, '_') == 0) {
                    ...
                    $consumptionRules["consumption.$key.unit"] = ['required', 'in:' . implode(',', config('panel.units.fcpu'))];
            ...
            return redirect()->route('products.show', $this->product->slug)->with('toast', [
                'class' => 'danger',
                'text' => 'Failed to create product range. Error: ' . $e->getMessage(),
            ]);
        }
    }
